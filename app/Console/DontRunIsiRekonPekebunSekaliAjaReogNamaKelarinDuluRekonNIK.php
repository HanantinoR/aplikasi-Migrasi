...
foreach ($array_hasil_cek_get_pekebun_proposal_psr_online as $key => $row) {
    ...
    if ($array_lookup_by_id_proposal[$row['id_proposal']] === null) {
        ...
        $query_get_pekebun_proposal_smart_psr = 'SELECT id,id_proposal,nik,nama_pekebun,luas_hektar FROM tbl_master_pekebun WHERE id_proposal = "' . $row['id_proposal_smart_psr'] . '" AND nama_pekebun LIKE "%' . $row['nama_pekebun'] . '%" ORDER BY id_proposal ASC LIMIT 1';
