...
$user = 'Script_Auto Rekon Pekebun';
...
$query_get_pekebun_proposal_psr_online = 'SELECT * FROM tb_pekebun WHERE id_proposal IN (' . $string_array_id_proposal_psr . ') ORDER BY id_proposal ASC LIMIT 1000000 OFFSET 71000';
    ...
    if ($array_lookup_by_id_proposal[$row['id_proposal']] === null) {
        ...
        if ($hasil_cek_get_pekebun_proposal_smart_psr->num_rows == 0) {
            ...
            $query_insert_log_migrasi_pekebun = 'insert into `pekebun_log` (
                                                ...
                                                "' . $id_pekebun . '",
                                                "' . $row['no_ktp'] . ' di migrasi oleh SCRIPT RDP - ' . $user . ' melalui aplikasi migrasi RDP",
                                                "' . $now . '",
                                                "' . $user . '"
                                                )';
            ...
            foreach ($array_hasil_get_data_legalitas_lahan_psr_online as $identifier => $baris) {
                ...
                $query_insert_log_legalitas_lahan = 'insert into `legalitas_lahan_pekebun_log` (
                                                ...
                                                "' . $id_legalitas_lahan_pekebun . '",
                                                "' . $id_proposal . '",
                                                "Legalitas ' . $row['no_ktp'] . ' di migrasi oleh RDP - ' . $user . ' melalui aplikasi migrasi RDP",
                                                "' . $now . '",
                                                "' . $user . '"
                                            )';
