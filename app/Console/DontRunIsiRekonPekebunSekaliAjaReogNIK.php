...
$query_get_pekebun_proposal_psr_online = 'SELECT * FROM tb_pekebun WHERE id_proposal IN (' . $string_array_id_proposal_psr . ') AND id_pekebun NOT IN ('.$string_array_id_pekebun_udah_rekon.') ORDER BY id_proposal ASC';
...
foreach ($array_hasil_cek_get_pekebun_proposal_psr_online as $key => $row) {
    ...
    if ($array_lookup_by_id_proposal[$row['id_proposal']] === null) {
        ...
        $no_ktp_potong_15_digit = substr($row['no_ktp'],0,15);
        ...
        $query_get_pekebun_proposal_smart_psr = 'SELECT id,id_proposal,nik,nama_pekebun,luas_hektar FROM tbl_master_pekebun WHERE id_proposal = "' . $row['id_proposal_smart_psr'] . '" AND nik LIKE "%' . $no_ktp_potong_15_digit . '%" ORDER BY id_proposal ASC LIMIT 1';
