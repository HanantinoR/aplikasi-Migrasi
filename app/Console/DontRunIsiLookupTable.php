...
$query_get_proposal_psr_online = 'SELECT id_proposal,id_koperasi,no_dokumen FROM tb_proposal ORDER BY no_dokumen ASC';

$query_get_proposal_smart_psr = 'SELECT id,id_lembaga_pekebun,nomor_proposal FROM tbl_master_proposal ORDER BY nomor_proposal ASC';

$query_get_lookup_rdp_lengkap = 'SELECT nomor_proposal FROM lookup_proposal_kelembagaan_pekebun WHERE id_proposal_smart_psr IS NOT NULL';
...
$hasil_cek_get_proposal_psr_online = mysqli_query($conn_psr_online, $query_get_proposal_psr_online);
$hasil_cek_get_proposal_smart_psr = mysqli_query($conn_smart_psr, $query_get_proposal_smart_psr);
$hasil_cek_get_lookup_proposal_lengkap_rdp = mysqli_query($conn_rdp, $query_get_lookup_rdp_lengkap);

$array_proposal = array();
$list_proposal_psr_online = array();
$list_proposal_data_lookup_lengkap = array();

foreach ($hasil_cek_get_proposal_psr_online as $key => $row) {
    $array_proposal[$row['no_dokumen']] = array(
        "nomor_proposal" => $row['no_dokumen'],
        "id_proposal_psr_online" => $row['id_proposal'],
        "id_kelembagaan_pekebun_psr_online" => $row['id_koperasi']
    );
    $list_proposal_psr_online[] = $row['no_dokumen'];
}

foreach ($hasil_cek_get_lookup_proposal_lengkap_rdp as $key => $row) {
    $list_proposal_data_lookup_lengkap[] = $row['nomor_proposal'];
}

mysqli_free_result($hasil_cek_get_lookup_proposal_lengkap_rdp);

// // Cek Dulu Ada Apa Engga Ntu Nomor Proposal di SMART-PSR
// var_dump($array_proposal);

echo ("Hasil Fetch Nomor Proposal Penyaluran \n");
foreach ($hasil_cek_get_proposal_smart_psr as $row) {
